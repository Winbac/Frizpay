...
const ChipIcon = () => (
  <svg width="28" height="22" viewBox="0 0 28 22" fill="none">
    <rect x="1" y="1" width="26" height="20" rx="3" stroke="#5ECC5E" strokeWidth="1.5" fill="none"/>
    <rect x="9" y="1" width="10" height="20" stroke="#5ECC5E" strokeWidth="1" fill="none" opacity="0.5"/>
    <rect x="1" y="7" width="26" height="8" stroke="#5ECC5E" strokeWidth="1" fill="none" opacity="0.5"/>
    <rect x="9" y="7" width="10" height="8" fill="#5ECC5E" opacity="0.2"/>
  </svg>
)

export default function VirtualCard({ balance = "$400.00", cardNumber = "6744 5698 472 0124", expiry = "09/24" }) {
  return (
    <div className="card-gradient rounded-2xl p-5 w-full relative overflow-hidden">
      ...
      <div className="relative z-10">
        ...
        {/* Balance */}
        <div className="mb-4">
          <p className="text-text-muted text-[10px] uppercase tracking-wider mb-0.5">Available Balance</p>
          <p className="text-white text-xl font-bold">{balance}</p>
        </div>

        {/* Card Number & Expiry */}
        <div className="flex items-center justify-between">
          <p className="text-text-soft text-xs tracking-widest font-mono">{cardNumber}</p>
          <p className="text-text-soft text-xs font-mono">{expiry}</p>
        </div>
      </div>
    </div>
  )
