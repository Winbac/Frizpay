...
const CustomTooltip = ({ active, payload, label }) => {
  if (active && payload && payload.length) {
    return (
      <div className="bg-bg-elevated border border-border-dark rounded-xl p-3 text-xs shadow-xl">
  ...
  return (
    <div className="bg-bg-card rounded-2xl p-5 border border-border-dark">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h3 className="text-white font-semibold text-sm">Income Analysis</h3>
        </div>
      ...
      <ResponsiveContainer width="100%" height={200}>
        <AreaChart data={data} margin={{ top: 5, right: 5, left: -20, bottom: 0 }}>
          ...
          <Area
            ...
            dot={false}
            activeDot={{ r: 5, fill: '#5ECC5E', stroke: '#fff', strokeWidth: 2 }}
          />
          <Area
            type="monotone"
            dataKey="expense"
